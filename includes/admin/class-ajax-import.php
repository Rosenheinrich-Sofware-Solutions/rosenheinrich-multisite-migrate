...
class Rmmigrate_Ajax_Import
{
    ...
    public static function import_local_chunk(): void
    {
        ...
        if ($chunk === false || $chunk === '') {
            ...
            if ($tmp !== '') {
                $chunk = (string) Rmmigrate_Filesystem::get_contents($tmp);
            }
        }
        if ($chunk === '') {
            self::import_error(__('Empty chunk.', 'rosenheinrich-multisite-migrate'), $err_ctx);
        }
        if (strlen($chunk) > 20971520) {
            self::import_error(__('Chunk exceeds maximum allowed size.', 'rosenheinrich-multisite-migrate'), $err_ctx);
        }

        Rmmigrate_Filesystem::put_contents($part_path, $chunk, FILE_APPEND | LOCK_EX);
        $bytes_written = Rmmigrate_Filesystem::exists($part_path)
            ? (int) Rmmigrate_Filesystem::filesize($part_path)
            : 0;
    ...
    public static function import_and_restore(): void
    {
        ...
        $via_worker = Rmmigrate_Request_Input::post_bool('via_worker');
        Rmmigrate_Logger::log_activity(
            'restore',
            sprintf(
        ...
        try {
            $result = Rmmigrate_Import_Service::import_and_restore($job_id, $via_worker);
            if ($via_worker) {
                wp_send_json_success($result);
                return;
            }
